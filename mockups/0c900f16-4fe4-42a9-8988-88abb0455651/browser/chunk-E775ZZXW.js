...
// src/app/core/frames-api.service.ts
var FramesApiService = class _FramesApiService {
  constructor() {
    this.http = inject(HttpClient);
  }
  /** Fetch a single frame plus its degraded-performance flag. */
  getFrame(date, mode) {
    return this.http.get("/api/frames", {
      params: { date, mode },
      observe: "response"
    }).pipe(map((res) => ({
      frame: this.toFrame(res.body),
      degraded: res.headers.get("X-Degraded-Performance") === "true"
    })));
  }
  /**
   * Prefetch & buffer every frame in `dates` (already-validated, available
   * dates) so playback runs from memory. The animation is flagged degraded if
   * any frame was served without caching.
   */
  getAnimation(dates, mode) {
    if (dates.length === 0) {
      return of({ frames: [], degraded: false });
    }
    return forkJoin(dates.map((d) => this.getFrame(d, mode))).pipe(map((results) => ({
      frames: results.map((r) => r.frame),
      degraded: results.some((r) => r.degraded)
    })));
  }
  /**
   * Flatten the backend's 2-D arrays into the flat, north-up `Frame` the
   * canvas renders. Backend row 0 is the southernmost band; the canvas draws
   * row 0 at the top, so rows are reversed.
   */
  toFrame(b) {
    ...
    for (let r = 0; r < rows; r++) {
      const src = rows - 1 - r;
      const valueRow = b.values[src] ?? [];
      const confRow = b.confidence[src] ?? [];
      for (let c = 0; c < cols; c++) {
        const idx = r * cols + c;
        grid[idx] = valueRow[c] ?? null;
        confidence[idx] = confRow[c] ?? 0;
      }
    }
    return { date: b.date, rows, cols, grid, confidence };
  }
  static {
    this.\u0275fac = function FramesApiService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _FramesApiService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FramesApiService, factory: _FramesApiService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FramesApiService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// src/app/core/dates-api.service.ts
var MSG_NO_DATA = "No data available for this date range yet";
var DatesApiService = class _DatesApiService {
  constructor() {
    this.http = inject(HttpClient);
  }
  bounds() {
    return this.http.get("/api/dates/bounds");
  }
  validate(start, end) {
    return this.http.get("/api/dates/validate", {
      params: { start, end }
    });
  }
  static {
    this.\u0275fac = function DatesApiService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _DatesApiService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _DatesApiService, factory: _DatesApiService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DatesApiService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();
...
var VisualizationComponent = class _VisualizationComponent {
  constructor() {
    this.framesApi = inject(FramesApiService);
    this.datesApi = inject(DatesApiService);
    this.frames = signal([]);
  ...
  onModeChange(mode) {
    ...
    this.mode.set(mode);
    this.loadAnimation(this.range(), keepIndex, wasPlaying);
  }
  /**
   * Validate the range (availability) then prefetch & buffer every available
   * frame for the current mode. Blocks the load with the correct inline state
   * when a range has no ingested data; degraded-performance is consumed
   * silently. `resumePlaying` restarts playback once buffering completes
   * (used by the mode toggle).
   */
  loadAnimation(range, resumeIndex = 0, resumePlaying = false) {
    this.pause();
    const token = ++this.loadToken;
    const mode = this.mode();
    this.datesApi.validate(range.start, range.end).subscribe({
      next: (result) => {
        if (token !== this.loadToken)
          return;
        const dates = result.dates ?? [];
        if (!result.valid || dates.length === 0) {
          this.frames.set([]);
          this.degraded.set(false);
          this.state.set(result.error === MSG_NO_DATA || dates.length === 0 ? "no-data" : "error");
          return;
        }
        this.framesApi.getAnimation(dates, mode).subscribe({
          next: ({ frames, degraded }) => {
            if (token !== this.loadToken)
              return;
            this.frames.set(frames);
            this.degraded.set(degraded);
            this.state.set("ready");
            this.currentIndex.set(Math.min(resumeIndex, Math.max(0, frames.length - 1)));
            if (resumePlaying) {
              this.play();
            }
          },
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(VisualizationComponent, { className: "VisualizationComponent", filePath: "src/app/visualization/visualization.component.ts", lineNumber: 40 });
})();
export {
  VisualizationComponent
};
//# sourceMappingURL=chunk-E775ZZXW.js.map

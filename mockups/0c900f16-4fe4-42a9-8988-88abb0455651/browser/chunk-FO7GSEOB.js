...
// src/app/core/ingestion-api.service.ts
var STATUS_MAP = {
  RUNNING: "running",
  SUCCESS: "success",
  FAILED: "failed"
};
var IngestionApiService = class _IngestionApiService {
  constructor() {
    this.http = inject(HttpClient);
  }
  /** Job history for the admin jobs table, newest first. */
  listJobs(limit = 50) {
    return this.http.get("/api/admin/ingestion/runs", {
      params: { limit }
    }).pipe(map((runs) => runs.map((r) => this.toJob(r))));
  }
  /** Trigger a daily ingestion run now (optionally for a specific date). */
  trigger(date) {
    return this.http.post("/api/admin/ingestion/run", date ? { date } : {}).pipe(map((r) => this.toJob(r)));
  }
  /** Trigger the one-time Climate Normals baseline import. */
  importNormals() {
    return this.http.post("/api/admin/ingestion/normals", {});
  }
  toJob(r) {
    ...
    return {
      ...
      source: "GHCN-Daily",
      status: STATUS_MAP[r.status] ?? "running",
  ...
  static {
    this.\u0275fac = function IngestionApiService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _IngestionApiService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _IngestionApiService, factory: _IngestionApiService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(IngestionApiService, [{
    type: Injectable,
...
var AdminJobsComponent = class _AdminJobsComponent {
  constructor() {
    this.ingestion = inject(IngestionApiService);
    this.jobs = signal([]);
  ...
  ngOnInit() {
    this.ingestion.listJobs().subscribe({
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminJobsComponent, { className: "AdminJobsComponent", filePath: "src/app/admin-jobs/admin-jobs.component.ts", lineNumber: 19 });
})();
export {
  AdminJobsComponent
};
//# sourceMappingURL=chunk-FO7GSEOB.js.map

...
// src/app/core/settings-api.service.ts
var SettingsApiService = class _SettingsApiService {
  ...
  update(updates) {
    return this.http.patch("/api/admin/settings", updates);
  }
  static {
    this.\u0275fac = function SettingsApiService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _SettingsApiService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _SettingsApiService, factory: _SettingsApiService.\u0275fac, providedIn: "root" });
  }
};
(() => {
...
var AdminSettingsComponent = class _AdminSettingsComponent {
  constructor() {
    this.api = inject(SettingsApiService);
  ...
  reload() {
    this.api.list().subscribe({
      next: (settings) => this.services.set(settings.map((s) => this.toCard(s))),
      error: () => this.services.set([])
    });
  }
  /** Map a masked SettingView onto the credential-card view model. */
  toCard(s) {
    return {
      key: s.key,
      label: s.label,
      description: s.configured ? `${s.service} \xB7 current value ${s.value ?? "\u2022\u2022\u2022\u2022"}` : `${s.service} \xB7 no value configured yet`,
  ...
  save(serviceKey) {
    const svc = this.services().find((s) => s.key === serviceKey);
    const value = svc?.fields[0]?.value.trim() ?? "";
    if (!value) {
      return;
    }
    this.api.update([{ key: serviceKey, value }]).subscribe({
      next: () => {
        this.savedKey.set(serviceKey);
        this.reload();
      },
      error: () => this.savedKey.set(null)
    });
  }
  static {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminSettingsComponent, { className: "AdminSettingsComponent", filePath: "src/app/admin-settings/admin-settings.component.ts", lineNumber: 23 });
})();
export {
  AdminSettingsComponent
};
//# sourceMappingURL=chunk-TNQHMWCO.js.map

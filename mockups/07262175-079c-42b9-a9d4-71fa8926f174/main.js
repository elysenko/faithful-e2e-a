...
function LayoutComponent_Conditional_10_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275conditional(0, ctx_r2.auth.isPlanner() ? 0 : -1);
    \u0275\u0275advance(2);
    \u0275\u0275classProp("planner", ctx_r2.auth.isPlanner());
    \u0275\u0275advance(2);
...
var LayoutComponent = class _LayoutComponent {
  constructor(auth) {
    this.auth = auth;
    this.allNav = [
      { label: "Projects", icon: "\u{1F3D7}\uFE0F", link: "/projects", scope: "auth" },
      { label: "Settings", icon: "\u2699\uFE0F", link: "/admin/settings", scope: "planner" }
    ];
    this.nav = computed(() => {
      const planner = this.auth.isPlanner();
      return this.allNav.filter((n) => n.scope === "planner" ? planner : true);
    });
    this.bottomNav = computed(() => this.nav().slice(0, 5));
    this.userName = computed(() => this.auth.user()?.name || this.auth.user()?.email || "Account");
    this.roleLabel = computed(() => this.auth.isPlanner() ? "Planner" : "Viewer");
  }
  ngOnInit() {
...
var PLANNER = { roles: ["planner"] };
var routes = [
  {
    path: "login",
    loadComponent: () => import("./login.component-NJ77U2RZ.js").then((m) => m.LoginComponent),
    data: { flow: { node: "login", entry: true, edgesTo: ["projects", "signup"] } }
  },
  {
    path: "signup",
    loadComponent: () => import("./signup.component-4LXVNGUZ.js").then((m) => m.SignupComponent),
    data: { flow: { node: "signup", edgesTo: ["projects", "login"] } }
  },
  {
    path: "",
    ...
    children: [
      ...
      // ---- Projects list (any logged-in user) ----
      {
        path: "projects",
        canActivate: [authGuard],
        loadComponent: () => import("./projects-list.component-DLVDFX3I.js").then((m) => m.ProjectsListComponent),
        data: { flow: { node: "projects", showInNavbar: true, edgesTo: ["project-detail"] } }
      },
      // ---- Project detail shell + child feature views ----
      {
        path: "projects/:id",
        canActivate: [authGuard],
        loadComponent: () => import("./project-detail.component-IER5XPD4.js").then((m) => m.ProjectDetailComponent),
        data: { flow: { node: "project-detail", edgesTo: ["dashboard", "schedule", "milestones", "budget", "siteplan", "projects"] } },
        children: [
          { path: "", pathMatch: "full", redirectTo: "dashboard" },
          {
            path: "dashboard",
            loadComponent: () => import("./dashboard.component-L7AUDMJM.js").then((m) => m.DashboardComponent),
            data: { flow: { node: "dashboard", edgesTo: ["schedule", "budget", "siteplan"] } }
          },
          {
            path: "schedule",
            loadComponent: () => import("./gantt.component-D6TKFSWD.js").then((m) => m.GanttComponent),
            data: { flow: { node: "schedule", edgesTo: ["dashboard"] } }
          },
          {
            path: "milestones",
            loadComponent: () => import("./milestones.component-GJAGBH3P.js").then((m) => m.MilestonesComponent),
            data: { flow: { node: "milestones", edgesTo: ["dashboard"] } }
          },
          {
            path: "budget",
            loadComponent: () => import("./budget-burndown.component-UBU6CG5Y.js").then((m) => m.BudgetBurndownComponent),
            data: { flow: { node: "budget", edgesTo: ["dashboard"] } }
          },
          {
            path: "siteplan",
            loadComponent: () => import("./siteplan.component-JD43Z43S.js").then((m) => m.SiteplanComponent),
            data: { flow: { node: "siteplan", edgesTo: ["dashboard"] } }
          }
        ]
      },
      // ---- Admin settings (planner only) ----
      {
        path: "admin/settings",
        canActivate: [authGuard, roleGuard],
        loadComponent: () => import("./settings.component-2YWKHINU.js").then((m) => m.SettingsComponent),
        data: { roles: PLANNER.roles, flow: { node: "settings", showInNavbar: true, scope: "planner", edgesTo: ["projects"] } }
      }
    ]
  },
  { path: "**", redirectTo: "" }

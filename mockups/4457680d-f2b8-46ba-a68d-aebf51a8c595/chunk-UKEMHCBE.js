...
var AuthService = class _AuthService {
  constructor(http, router) {
    ...
    this._role = signal(localStorage.getItem(ROLE_KEY) ?? null);
    this.isAuthenticated = computed(() => !!this._token());
    this.token = this._token.asReadonly();
    this.role = this._role.asReadonly();
  }
  login(email, password) {
    return this.http.post(`${this.apiUrl}/auth/login`, { email, password }).pipe(tap((res) => this.persistSession(res.token, res.user)));
  }
  signup(email, password) {
    return this.http.post(`${this.apiUrl}/auth/signup`, { email, password }).pipe(tap((res) => this.persistSession(res.token, res.user)));
  }
  /**
   * Demo Mode bypass — lets reviewers (and the screenshot capture system) inspect the
   * full authenticated UI without a live backend. Seeds a mock admin session and routes
   * straight to the main app.
   */
  demoLogin() {
    this.persistSession("demo-token", { email: "admin@demo.dev", role: "admin" });
    this.router.navigate(["/tasks"]);
  }
  logout() {
    [TOKEN_KEY, ACCESS_TOKEN_KEY, USER_KEY, AUTH_FLAG_KEY, ROLE_KEY].forEach((k) => localStorage.removeItem(k));
...
//# sourceMappingURL=chunk-UKEMHCBE.js.map

...
var EventDetailComponent = class _EventDetailComponent {
  constructor(auth, route, router, api) {
    this.auth = auth;
    this.route = route;
    this.router = router;
    this.api = api;
  ...
  /** Fetch the event with counts and (when authed) the caller's RSVP status. */
  load() {
    ...
    this.api.getEvent(id).subscribe({
      next: (event) => {
        ...
        this.events.set([__spreadProps(__spreadValues({}, event), { myRsvpStatus: myRsvp?.status ?? null })]);
  ...
  onRsvp() {
    ...
    this.api.rsvp(this.eventId()).subscribe({
      next: () => {
        this.acting.set(false);
        this.load();
      },
      error: () => {
        this.error.set("Could not RSVP. Please try again.");
        this.acting.set(false);
      }
    });
  }
  onCancel() {
    this.acting.set(true);
    this.api.cancelRsvp(this.eventId()).subscribe({
      next: () => {
        this.acting.set(false);
        this.load();
      },
      error: () => {
        this.error.set("Could not cancel your RSVP. Please try again.");
        this.acting.set(false);
      }
    });
  }
  ...
  static {
    this.\u0275fac = function EventDetailComponent_Factory(t) {
      return new (t || _EventDetailComponent)(\u0275\u0275directiveInject(AuthService), \u0275\u0275directiveInject(ActivatedRoute), \u0275\u0275directiveInject(Router), \u0275\u0275directiveInject(ApiService));
    };
  }
  static {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(EventDetailComponent, { className: "EventDetailComponent", filePath: "src/app/pages/event-detail/event-detail.component.ts", lineNumber: 21 });
})();
export {
  EventDetailComponent
};
//# sourceMappingURL=event-detail.component-AAV3H2EN.js.map

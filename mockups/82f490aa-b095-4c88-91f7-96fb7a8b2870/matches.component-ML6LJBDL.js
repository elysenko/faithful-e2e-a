...
var MatchesComponent = class _MatchesComponent {
  ...
  ngOnInit() {
    this.matchService.list().subscribe({
      // Guard against null partners so the template never dereferences `m.user`.
      next: (matches) => this.matches.set((matches ?? []).filter((m) => !!m.user)),
...
//# sourceMappingURL=matches.component-ML6LJBDL.js.map

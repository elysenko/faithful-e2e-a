...
var ReportDetailComponent = class _ReportDetailComponent {
  constructor() {
    ...
    this.report = computed(() => this.reports().find((r) => r.id === this.reportId) ?? EMPTY_REPORT);
  }
  ngOnInit() {
    this.admin.reports().subscribe({
      next: (reports) => {
        ...
        if (this.reportId && !list.some((r) => r.id === this.reportId)) {
  ...
  ban() {
    ...
    if (!r.id)
      return;
    this.admin.ban(r.reportedId).subscribe({
      next: () => {
        this.admin.review(r.id).subscribe({ error: () => {
        } });
        this.resolution.set("ban");
        this.toast.push("message", "User banned", `${r.reportedName} has been removed.`);
      },
      error: () => {
      }
    });
  ...
  dismiss() {
    ...
    if (!r.id)
      return;
    this.admin.review(r.id).subscribe({
      next: () => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ReportDetailComponent, { className: "ReportDetailComponent", filePath: "src/app/features/admin/report-detail.component.ts", lineNumber: 27 });
})();
export {
  ReportDetailComponent
};
//# sourceMappingURL=report-detail.component-452LWXOV.js.map
